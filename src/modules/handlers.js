...
const saveInput = (newItem, type) => {
  if (type === 'new-task' || type === 'task' || type === 'due-date') {
    if (type === 'new-task') {
      ...
      if (!newItem.instance.tags.includes(currentDesk[0]))
        newItem.instance.addTag(String(currentDesk[0]));
  ...
  if (type === 'new-checklist' || type === 'checklist') {
    const id =
      type === 'new-checklist'
        ? newItem.instance.taskId
        : newItem.id.split('-');

    const task = findItemId(
      tasks,
      Number(type === 'new-checklist' ? id : id[0])
    );
  ...
  if (type === 'new-list' || type === 'list') {
    ...
    if (newList) {
      ...
      if (type === 'list') {
        ...
        tasks.map((task) => {
          if (
            task.tags.includes(String(listToUpdate.title.toLowerCase().trim()))
          )
            task.updateTag(
              listToUpdate.title.toLowerCase().trim(),
              String(newList).toLocaleLowerCase().trim()
            );
        });
...
const deleteItem = (event, id, type) => {
  ...
  const itemToDelete = findItemId(
    array,
    Number(type === 'checklist' ? id[0] : id)
  );
  ...
  if (type === 'list') {
    tasks.map((task) => {
      if (task.tags.includes(String(itemToDelete.title.toLowerCase().trim())))
        task.deleteTag(itemToDelete.title.toLowerCase().trim());
    });
...
const checkboxState = (id, type, btn) => {
  const checkbox = selectNode(
    `#${btn}-${btn === 'checkbox' ? 'task' : 'item'}-${id}`
  );
  ...
  const task =
    type === 'checkbox-state'
      ? findItemId(tasks, Number(id))
      : findItemId(findItemId(tasks, Number(id[0])).checklist, Number(id[1]));
...
export const addNewItem = (event, type, id) => {
  ...
  const input =
    type === 'new-task'
      ...
      : type === 'new-list'
      ? newListUi(instance)
      : addNewCheckUi({ id }, instance, 'checklist', null);
...
const editItem = (id, type) => {
  const input =
    type === 'task'
      ...
      : type === 'list'
      ? textInputUi({ type }, 'edit', false, 15)
      : type === 'checklist'
      ? textInputUi({ type, id }, 'edit', false, 20)
      : dateInputUi({ id });

  const nodes =
    type === 'task'
      ? appendInput(
          `#checkbox-wrapper-${id} > label`,
          `#checkbox-wrapper-${id}`,
          input,
          true
        )
      ...
      : type === 'checklist'
      ? appendInput(
          `#checklist-item-wrapper-${id} > label`,
          `#checklist-item-wrapper-${id}`,
          input,
          true
        )
      : appendInput(
