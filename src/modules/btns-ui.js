...
export const btnsUi = (type, dataType, textContent, idPrefix, attrs) => {
    ...
    if (textContent === 'edit') {
        btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24" id="svg-${idPrefix}-${type.id}" class="svg-btns-edit" data-number="${type.id}" data-type="${dataType}" data-btn="${textContent}"><path d="m19.725 9.4-4.9-4.875 1.25-1.275q.75-.75 1.812-.775 1.063-.025 1.913.775l1.225 1.225q.85.8.787 1.85-.062 1.05-.812 1.8ZM18.3 10.825 7.35 21.8H2.425v-4.9L13.4 5.95Z" id="svg-${idPrefix}-path-${type.id}" class="svg-btns-edit" data-number="${type.id}" data-type="${dataType}" data-btn="${textContent}"/></svg>`;
        ...
        btn.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" height="24" width="24" data-btn="${textContent}" data-type="${dataType}"><path d="m12 16.35-7-7 2.4-2.375 4.6 4.6 4.6-4.6L19 9.35Z" data-btn="${textContent}" data-type="${dataType}"/></svg>`;
    } else {
