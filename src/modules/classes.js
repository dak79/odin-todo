...
class TaskBase {
  ...
  updateTime() {
    if (isToday(this.dueDate) && !this.tags.find((tag) => tag === 'today'))
      this.addTag('today');

    if (
      isThisWeek(this.dueDate, { weekStartsOn: 1 }) &&
      !this.tags.find((tag) => tag === 'this-week')
    )
      this.addTag('this-week');

    if (
      (isAfter(this.dueDate, endOfWeek(new Date(), { weekStartsOn: 1 })) ||
        !this.dueDate) &&
      !this.tags.find((tag) => tag === 'anytime')
    )
      this.addTag('anytime');

    if (
      isBefore(this.dueDate, new Date()) &&
      this.dueDate &&
      !this.tags.find((tag) => tag === 'late' || tag === 'today')
    )
      this.addTag('late');
  }
