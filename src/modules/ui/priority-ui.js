...
export const updatePriorityUi = (task, wrapperMsg, isChanging) => {
  ...
  if (wrapperMsg.querySelector(`#low-priority-${task.id}`))
    wrapperMsg.querySelector(`#low-priority-${task.id}`).remove();

  if (wrapperMsg.querySelector(`#high-priority-${task.id}`))
    wrapperMsg.querySelector(`#high-priority-${task.id}`).remove();
